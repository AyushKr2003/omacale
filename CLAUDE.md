...
| `KeyNav.qml` (+ the cursor in `PopoutContent.qml`, `StateLayer.focused`) | no Caelestia original (its popouts are pointer-only): Omarchy's `Ui/PanelKeyCatcher.qml` keys, drawn with M3's focus state (the hover veil at 10%, the switch's circle around its handle, the slider's pressed handle) plus M3's focus indicator, `FocusRing`: a 3px `m3secondary` outline 2px outside the control, following its radius. The veil alone was too faint to find the cursor |
...
| `WallpaperList.qml`, `WallpaperItem.qml` (+ the `>wallpaper `/`>theme ` modes in `Launcher.qml`) | `modules/launcher/WallpaperList.qml`, `items/WallpaperItem.qml`, `ContentList.qml`. One deliberate difference: with exactly two entries Caelestia shows only the current one; Omacale shows both side by side (`pair`), standing still, the selection only changing which one is big. PathView adds `preferredHighlightBegin` to every position whenever it snaps, even with `NoHighlightRange`, so the pair zeroes it |
...
- **Every size goes through the UI scale.** `Tk`'s tokens are already scaled (Caelestia's `appearance.{font,padding,spacing,rounding}.scale` under Omacale's master `uiScale`, which follows Omarchy's `Style.fontScale` by default). A size with no token is `Tk.px(n)`, a point size `Tk.font(n)`; never a bare number. Leave hairlines (1-2px lines and borders, the `FocusRing`), counts, angles, ratios and pointer thresholds bare. The frame's thickness and smoothing (`Tk.border`, `smoothing`) are user px and don't scale; its corner (`Tk.borderRounding`) follows the rounding scale. At 1x every token is exactly Caelestia's.
...
- **IPC** is the `omacale` target in `Bar.qml`: `launcher`, `dashboard`, `session`, `settings`, `sidebar`, `utilities`, `toggles`, `overview`, `dashboardTab(tab: string)`, `settingsPage(page: string)`, `popout(name: string)`, `barFocus`, `wallpapers`, `themes`, `menu`, `windowInfo`, `scale(value: string): string`, `close`. IPC functions **must have typed args and a typed return (`: void`, `: string`, ...)** or Quickshell drops the whole target. Call with `omarchy-shell omacale <fn>` (or `qs -p /usr/share/omarchy/shell ipc call omacale <fn>`).
...
- **Stable Omarchy (4.0.x) is older than `quattro` in places Omacale reaches.** Its `MenuModel.js` has no `isDisabled` and a `displayRow` without `disabledResults` (the engine wrapper's `hasDisabled` picks the signature; called the new way every row came back id-less and drew blank), its recorder writes the marker to `/tmp/omarchy-screenrecord-filename` (`RecordService.markerProbe`), and it has no video backgrounds (`switcher.sh` lists images only without `BackgroundMedia.qml`). To check a change against stable: `git -C omarchy-repo archive v4.0.4 | tar -x -C <dir>` and `OMARCHY_PATH=<dir> scripts/upstream-check`.
...
- **A keyboard-opened popout is held (`ScreenScope.popoutKeys`) and primes focus.** `win` is always mapped, so OnDemand alone never hands it the keyboard: `primeFocus()` sets Exclusive for 120ms, then OnDemand plus `pop.forceActiveFocus()`, as Omarchy's `KeyboardPanel` does. That Exclusive→OnDemand switch **clears the `HyprlandFocusGrab`**, which used to `closeAll()` the popout the moment it opened; for 400ms after a prime (`primeSettle`) a cleared grab is re-taken instead. Pointer-opened popouts never set `popoutKeys`, so hover/click stay Caelestia's.
- **Keyboard navigation is a cursor over `navTarget`s, not `activeFocus` per control.** `PopoutContent.navTargets()` walks the page for items with `navTarget` (`StateLayer`, `MSwitch`, `MSlider`) in reading order on every key; `step()` moves by index, so a Repeater rebuilding rows under it (networks, devices) costs only the highlight. Enter/Space call `navActivate()` (`StateLayer` ripples and emits `clicked(null)`, so an `onClicked` that reads its mouse argument would break -- keep them argument-free), Left/Right call `navAdjust()` where there is one. Page keys are `navText(t)` on the page's root and `navDelete()` on a row. A destroyed cursor target is replaced by whatever now sits at `cursorIndex` (`cursorRestore`), so a refresh doesn't drop the highlight. `focused` is only set while `keyMode`, so the pointer never sees it. Controls only count once visible, a few frames into the popout's entry, so the first stop is found by `cursorSeek`, not at load.
- **The drawers share one keyboard cursor, `ScreenScope`'s `drawerNav` (`NavCursor`).** The dashboard opened by shortcut (`dashShortcut`), the sidebar, and utilities opened by shortcut (`utilShortcut`) make `drawerKeys` true, which primes the keyboard onto `drawerKeyItem`; hover-opened drawers never take keys, and a shortcut-opened one isn't handed to the pointer (Caelestia's hover rule, which drops `dashShortcut`/`utilShortcut` and with them the keyboard) while the cursor is showing -- the keys, a held one included, would land in the window behind. The cursor moves *spatially* (h/l sideways across cards and grids, j/k up and down), walks only each drawer's `navRoots` (the dashboard's are the tab bar and the page on show -- its other tabs sit clipped beside it in the strip), reveals stops in vertical Flickables, and appears on the first key rather than on open, so mouse users never see it. The sidebar and utilities share a panel, so while the sidebar is open utilities is a root too. Settings keeps its own focus and Escape, and hands every other key to `drawerKey` through `keyHook`. A switch whose whole row is the toggle (`RowToggle`) sets `MSwitch.inRow`, so the row is the one stop. A stop can act on focus alone (`navOnFocus`): the dashboard's tabs open as the cursor lands on them, the first key lands on the current tab rather than the first, and a tab change keeps a cursor that is still on the tab bar (`onRootsChanged` only clears a cursor outside the new roots). Drawer keys: Tab / 1..4 dashboard tabs, `n`/`p` media, `d` DND and Shift+X clear all in the sidebar, `x` dismisses a notification or group and deletes a recording (through its dialog), `/` search and Backspace back in Settings. How it picks a stop: stops in line with the cursor come first; sideways only within 45 degrees and never into or back to the stop the cursor sits in or holds (a notification and its action buttons); through the vertical list the cursor is in before out of it (j/k run down the notifications rather than to the clear-all button over them), and when leaving, stops scrolled out of another list are skipped. A `navRow` (the dashboard's tab bar) is its own lane -- h/l stay in it, and j/k into it land on its `navEnter()` stop (the open tab) rather than whichever tab is above, since landing on a tab opens it. A sliding strip gives `navShiftX` (how far it still has to go) so a key pressed mid-slide aims at the settled page. A cursor whose stop is destroyed (every notification change rebuilds the rows) is restored by `navKey` (`notif:<id>:<timestamp>[:action]`, `group:<app>`), else to the next surviving stop that was after it in its list, else the nearest in that list. Disabled subtrees (a group on its way out) hold no stops.
- **The bar focus mode (`SUPER+CTRL+0`, IPC `barFocus`) is `ScreenScope.barFocus` plus `BarContent.navStops()`.** Stops are rebuilt on every key, top to bottom (logo, workspace cells via `Workspaces.navItems()`, active window, plugin-pill slots, tray items, clock, status icons, power), each with its `act`. The cursor is one pill in `BarContent` (`barCursor`) that moves between them, not a focus state per item: Caelestia's workspace `ActiveIndicator` stretch (the leading edge on defaultSpatial, the trailing one 1.5x slower), drawn as an M3 focus indicator (2px `m3primary` outline, 14% tint), `Tk.barInner` wide so it sits on the pills, and hugging its item's height. Anything that opens a drawer or a hosted panel leaves bar focus first (`leaveFor`, `openPlugin`); a popout opened from it is `popoutKeys`, and Escape there (`popout = ""`) gives the keys back to the bar. A workspace switch from the bar goes through `ScreenScope.switchWorkspace`, which re-takes the grab Hyprland clears on the switch (`wsSettle`) instead of reading it as a click away. The compact tray and plugin overflow open while the cursor is in them.
- **`Sys.activeToplevel` is null right after a shell restart** until Hyprland sends a focus event, so the active window counts as absent (bar says "Desktop", `togglePanelAt left 1` finds nothing) until focus moves once.
- **Unmapping one of our surfaces takes the keyboard from the panel that holds it.** When the toast window goes away (the sidebar suppresses the toasts, or the last one times out under an open drawer), Hyprland hands the keyboard to a window even though the drawer's layer has it, so h/j/k/l went to the window behind. `toastWin.onVisibleChanged` calls `ScreenScope.retakeKeys()` (re-prime or re-grab). Any new surface that maps and unmaps while a panel has the keys needs the same.
- **The live plugin reload keeps already-compiled components.** "Local plugin changed, reloading" can leave an edited component (seen with `NavCursor.qml`) running its old code, so a test after an rsync can be testing the previous version. `omarchy-restart-shell` before trusting a behaviour test.
...
- **A failed health check hands the plugin back to Omarchy automatically** (`remove`), sets `<lock|notifs>.autoFellBack` in Config so the next start doesn't reinstall into the same fault, and sends a notification. Installing from Settings (or the installer) clears it. The mark only holds for the Omacale version it was set under (`fellBackVersion`): an upgrade may be the fix, so a new version tries once more, and falls back again if it has to. Fail safe, never fail closed.
- **Omarchy never retries a service that failed to load**, and QML caches the failed component, so a lock clone whose wrapper didn't fit stays dead after its files are repaired -- until the shell restarts. `lock-screen install --heal` (what `LockService` and the watchdog run) restarts the shell, detached and a few seconds late, when it repaired a clone while the `lock` IPC was dead; with no lock service there is no lock to lose. A shell killed in the middle of a plugin hot reload has segfaulted, hence the delay.
...
- **The contract is checked before any write, both ways**: every property and signal handler the stock `Service.qml` sets on its `LockView` must exist on the wrapper, and everything the wrapper sets on `StockLockView` must exist on the installed Omarchy's `LockView.qml`, or the script refuses (`tests/test-restore.sh` case Q). Adding a property upstream means updating `assets/lock/LockView.qml`, and `LockUi.qml` if it wants it.
- **Omacale runs on stable Omarchy 4.0.x too, whose lock is older than `quattro`'s.** A property Omarchy's view gained after 4.0.x (`displaysBlank`, `powerSaverActive`, `videoPosterPath`) goes in the wrapper's `stockOptional` and is bound only where the view has it: assigned directly, it is a load error on 4.0.x that takes the clone's `Service.qml` down, and with it the `lock` IPC and the lock itself (`lock status`: "Target not found"). Likewise 4.0.x has no `Ui/BackgroundMedia.qml`, so `LockUi` falls back to a plain `Image`, as the 4.0.x view does.
- **`LockUi.qml` reaches everything through `view`** (the wrapper) and never assumes it is set: the Loader assigns it a frame late.
- **Video backgrounds are a poster plus OWE's feed**, as in the stock view: the shell's `BackgroundMedia` draws stills only, so `LockUi` gives it `view.videoPosterPath` and loads the live feed from `view.feedSurfaceUrl` (the clone's `LockFeedSurface.qml`, `import Owe.LockFeed`). The feed is never blurred. In `lock preview` screenshots only the poster shows, for the stock view too; the moving feed is only visible on a real lock.
